...
import { type PortfolioData, type ColorPalette } from "@/types/portfolio";
...
function PublicPortfolioView({ portfolio }: { portfolio: PortfolioData }) {
...
export default async function PublicPortfolioPage({ params }: { params: { id: string } }) {
    ...
    const { success, data, error } = await getPublicPortfolioAction(portfolioId);
            ...
            <div className="flex flex-col min-h-screen">
                ...
                <main className="flex-grow flex items-center justify-center text-center p-4">
                    <div>
                        ...
                        <p className="text-muted-foreground mt-2">{error || "The portfolio you are looking for does not exist or is private."}</p>
    ...
    return (
        <Suspense fallback={<PortfolioSkeleton />}>
            <PublicPortfolioView portfolio={data} />
        </Suspense>
    )
}
