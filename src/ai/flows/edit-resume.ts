...
1.  Modify the resume's HTML content according to the prompt. Your output must be the **entire**, updated resume as a single block of clean, semantic HTML with inline CSS that adheres to the A4 sizing rule above.
...
3.  **Template Redesigns:** If the user asks to apply a template (without providing one), redesign the resume, or change the layout, you should completely redesign the HTML and inline CSS to create a professional, industry-standard resume that respects the A4 sizing rule.
