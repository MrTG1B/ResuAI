...
const AnalyzeCertificateOutputSchema = z.object({
  ...
  date: z.string().optional().describe("The date the certificate was issued or obtained (e.g., 'May 2023' or '2023-05-15'). If no date is found, this should be an empty string."),
  credentialUrl: z.string().optional().describe("A URL to the credential if one is found on the certificate. If no URL is found, this should be an empty string."),
});
...
- Extract the date of issuance or completion. If no date is present, return an empty string for the date field.
- If a URL for verifying the credential is present, extract it. If no URL is found, return an empty string for the credentialUrl field.`,
